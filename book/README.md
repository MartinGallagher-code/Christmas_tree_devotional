...
| `EDITING-WORD.md` | **Edit the whole book in Word.** Run `manuscript.py extract`, edit `manuscript.docx`, run `manuscript.py sync`. The easiest way to change the words. |
| `manuscript.py` | Converts `book.html` ⇄ `manuscript.docx` so you can edit in Word and sync your changes back. |
| `manuscript.docx` | The book as an editable Word document (generated by `manuscript.py extract`). |
| `EDITING.md` | How to edit the text directly in `book.html` without breaking the layout. |
| `README.md` | This file. |
...
**The easiest way — edit in Word.** See [`EDITING-WORD.md`](EDITING-WORD.md).
Run `./manuscript.py extract` to get `manuscript.docx`, edit the words in
Word/Pages/Google Docs, then run `./manuscript.py sync` to fold your changes
back into `book.html` (only the blocks you changed are rewritten). Rebuild
with `./build.sh`.

**Editing the HTML directly** is still supported — see [`EDITING.md`](EDITING.md).
